...
export const ConcertsList = ({ title, concerts }: { title: string; concerts: Concert[] }) => {
  return (
...
export const ConcertsSection = ({ className }: { className?: string }) => {
  return (
    <div className={clsx("flex flex-col gap-8 items-center", className)}>
      <ConcertsList title="Upcoming Concerts" concerts={upcomingConcert} />
      <p className="text-heading-1 font-light pr-[124px]">ф</p>
      <ConcertsList title="Past Concerts" concerts={pastConcert} />
    </div>
  );
};
